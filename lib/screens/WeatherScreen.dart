...
class _WeatherScreenState extends State<WeatherScreen> {
  ...
  void initState() {
    ...
    if (widget.cityName != null && widget.cityName!.isNotEmpty) {
      ...
      _displayCity = widget.cityName ?? 'Your Location';
  ...
  void _extractForecastData(Map<String, dynamic> weatherData) {
    ...
    for (int i = 0; i < timeList.length; i++) {
      if (i > 0) { // Skip today (index 0) as we show it separately
        forecastList.add({
          'date': timeList[i],
          'day': _getDayOfWeek(DateTime.parse(timeList[i])),
          'temp_max': tempMaxList.isNotEmpty && i < tempMaxList.length ? tempMaxList[i]?.toDouble() ?? 0.0 : 0.0,
          'temp_min': tempMinList.isNotEmpty && i < tempMinList.length ? tempMinList[i]?.toDouble() ?? 0.0 : 0.0,
          'weather_code': weatherCodeList.isNotEmpty && i < weatherCodeList.length ? weatherCodeList[i] ?? 0 : 0,
          'precipitation': precipitationList.isNotEmpty && i < precipitationList.length ? precipitationList[i]?.toDouble() ?? 0.0 : 0.0,
          'wind_speed': windSpeedList.isNotEmpty && i < windSpeedList.length ? windSpeedList[i]?.toDouble() ?? 0.0 : 0.0,
        });
      }
    }
    ...
    setState(() {
      _forecastData = {
        ...
        'today': {
          'temp_max': tempMaxList.isNotEmpty ? tempMaxList[0]?.toDouble() ?? 0.0 : 0.0,
          'temp_min': tempMinList.isNotEmpty ? tempMinList[0]?.toDouble() ?? 0.0 : 0.0,
          'weather_code': weatherCodeList.isNotEmpty ? weatherCodeList[0] ?? 0 : 0,
        }
      };
    });
  }
  ...
  Map<String, dynamic>? getCurrentWeather() {
    ...
    if (_weatherData!['hourly'] != null &&
      ...
      humidity = _weatherData!['hourly']['relativehumidity_2m'][0]?.toDouble() ?? 50.0;
    }
    ...
    return {
      ...
      'wind': {
        'speed': current['windspeed']?.toDouble() ?? 0.0,
      },
    };
  }
        ...
        children: [
            ...
            children: [
              ...
              if (currentWeather != null)
                Padding(
                  ...
                  child: Icon(
                    _getWeatherIcon(currentWeather['weather'][0]['main'] == 'Clear' ? 0 : 1),
                    size: 70,
                    color: _getWeatherIconColor(currentWeather['weather'][0]['main'] == 'Clear' ? 0 : 1),
                  ),
                ),
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    if (currentWeather != null)
                      Text(
                        currentWeather['weather'][0]['main'] ?? 'Sunny',
                        style: const TextStyle(
                          fontSize: 18,
                          color: Colors.grey,
                        ),
          ...
          if (currentWeather != null)
            Row(
              ...
              children: [
                _buildWeatherDetail(Icons.water_drop, '${currentWeather['main']['humidity']?.round() ?? 0}%', 'Humidity', Colors.blue),
                _buildWeatherDetail(Icons.air, '${currentWeather['wind']['speed']?.toStringAsFixed(1) ?? '0.0'} km/h', 'Wind', Colors.green),
                _buildWeatherDetail(Icons.water, '10%', 'Rain', Colors.blue),
              ],
            ),
        ],
      ),
  ...
  Widget _buildHeatWarningCard() {
    ...
    if (temp < 35) return const SizedBox.shrink(); // Only show warning for high temps
    ...
    return SizedBox(
      ...
      child: ListView.builder(
        ...
        itemBuilder: (context, index) {
          ...
          final displayTime = hour == 12 ? '12 PM' :
          hour > 12 ? '${hour - 12} PM' :
          ...
          return Container(
            ...
            padding: const EdgeInsets.symmetric(vertical: 15, horizontal: 16), // Reduced horizontal padding
            decoration: BoxDecoration(
            ...
            child: Column(
              ...
              children: [
                ...
                Icon(
                  ...
                  color: index == 0 ? _getWeatherIconColor(weatherCode) : Colors.grey,
    ...
    return Column(
      children: forecastList.map<Widget>((dayData) {
        return _buildDailyItem(
          ...
          dayData['precipitation'] > 0 ? '${dayData['precipitation']} mm' : null,
        );
      }).toList(),
    );
  }

  Widget _buildDailyItem(String day, IconData icon, String condition, String temp, String? precipitation) {
    return Container(
          ...
          Row(
            children: [
              ...
              if (precipitation != null) ...[
                ...
                Text(precipitation, style: const TextStyle(color: Colors.blue, fontSize: 12)),
              ],
            ],
          ),
  ...
  void _navigateToHome(BuildContext context) {
    ...
    if (user != null) {
          ...
          Navigator.pushAndRemoveUntil(
            ...
            MaterialPageRoute(
              builder: (context) => HomeScreen(
                userLocation: userData['location'] ?? widget.cityName ?? 'Your Location',
                userName: userData['name'] ?? 'Farmer',
                selectedCrops: List<String>.from(userData['selectedCrops'] ?? ['Wheat']),
                primaryCrop: userData['primaryCrop'] ?? 'Wheat',
              ),
            ),
                (route) => false,
          );
      ...
      Navigator.pushAndRemoveUntil(
        ...
        MaterialPageRoute(
          builder: (context) => HomeScreen(
            userLocation: widget.cityName ?? 'Your Location',
            userName: 'Farmer',
            selectedCrops: ['Wheat'],
            primaryCrop: 'Wheat',
          ),
